...
describe('mergeKeyValues', () => {
  it('places global entries before local ones (local overrides on dup keys downstream)', () => {
  ...
  it('handles undefined global/local', () => {
    ...
    expect(
      mergeKeyValues([{ key: 'X', value: '1' }], undefined)
    ).toHaveLength(1)
  })
})
...
describe('maskSecretsDeep', () => {
  it('masks secret substrings in nested objects/arrays', () => {
    ...
    const input = {
      ...
      headers: { Authorization: 'Bearer sk_live_123', Accept: 'application/json' },
  ...
  it('is a no-op when there are no secrets', () => {
    const input = { a: 'b' }
    expect(maskSecretsDeep(input, new Set())).toEqual({ a: 'b' })
  })
})
...
describe('isResolvedUrlSafe', () => {
  ...
  it('rejects the cloud metadata endpoint', () => {
    expect(isResolvedUrlSafe('http://169.254.169.254/latest/meta-data').safe).toBe(
      false
    )
    expect(isResolvedUrlSafe('http://metadata.google.internal/').safe).toBe(false)
  })
