...
describe('Block Instrumentation Schema', () => {
  ...
  it('emits workspace.id and workspace.name (UAT-04)', () => {
    const result = runLoggerScript(
      `logger.info('TestWorkspace - Block Executed', { workspace: { id: 'ws-1', name: 'TestWorkspace' }, workflow: { id: 'wf-abc', name: 'My Flow', schema_version: '2', execution_id: 'sess-xyz', version_id: 'hist-1' }, typebot_block: { id: 'b-1', type: 'webhook' } });`
    )
    expect(result.workspace).toEqual({ id: 'ws-1', name: 'TestWorkspace' })
  })

  it('emits workflow.name and workflow.schema_version (UAT-02, UAT-03)', () => {
    const result = runLoggerScript(
      `logger.info('TestWorkspace - Block Executed', { workspace: { id: 'ws-1', name: 'TestWorkspace' }, workflow: { id: 'wf-abc', name: 'My Flow', schema_version: '2', execution_id: 'sess-xyz', version_id: 'hist-1' }, typebot_block: { id: 'b-1', type: 'webhook' } });`
    )
  ...
  it('emits typebot_block.id and typebot_block.type as nested object (BLOCK-02)', () => {
    const result = runLoggerScript(
      `logger.info('TestWorkspace - Block Executed', { workspace: { id: 'ws-1', name: 'TestWorkspace' }, workflow: { id: 'wf-1', name: 'My Flow', schema_version: '2', execution_id: 'sess-1', version_id: 'hist-1' }, typebot_block: { id: 'block-id-123', type: 'condition' } });`
    )
  ...
  it('workflow.schema_version is emitted as string, not number (UAT-03)', () => {
    const result = runLoggerScript(
      `logger.info('TestWorkspace - Block Executed', { workspace: { id: 'ws-1', name: 'TestWorkspace' }, workflow: { id: 'wf-1', name: 'My Flow', schema_version: '2', execution_id: 'sess-1', version_id: 'hist-1' }, typebot_block: { id: 'b-1', type: 'webhook' } });`
    )
    expect(typeof (result.workflow as any).schema_version).toBe('string')
  })

  it('emits workflow.version_id (HIST-01)', () => {
    const result = runLoggerScript(
      `logger.info('TestWorkspace - Block Executed', { workspace: { id: 'ws-1', name: 'TestWorkspace' }, workflow: { id: 'wf-1', name: 'My Flow', schema_version: '2', execution_id: 'sess-1', version_id: 'clxyz123abc' }, typebot_block: { id: 'b-1', type: 'webhook' } });`
    )
    expect((result.workflow as any).version_id).toBe('clxyz123abc')
  })
})
