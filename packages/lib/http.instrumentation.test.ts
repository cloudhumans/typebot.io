...
describe('HTTP Instrumentation Schema', () => {
  it('success path emits workspace, workflow, and http context (HTTP-01, UAT-04)', () => {
    const result = runLoggerScript(
      `logger.info('TestWS - HTTP Request Executed', { workspace: { id: 'ws-1', name: 'TestWS' }, workflow: { id: 'wf-1', name: 'Flow', schema_version: '2', execution_id: 'sess-http-1', version_id: 'hist-1' }, http: { url: 'https://example.com/api', method: 'POST', status_code: 200, duration: 142 } });`
    )
  ...
  it('error path emits http.url, http.method, http.status_code at warn level (HTTP-02)', () => {
    const result = runLoggerScript(
      `logger.warn('TestWS - HTTP Request Error', { workspace: { id: 'ws-1', name: 'TestWS' }, workflow: { id: 'wf-1', name: 'Flow', schema_version: '2', execution_id: 'sess-http-1', version_id: 'hist-1' }, http: { url: 'https://example.com/api', method: 'GET', status_code: 404, duration: 98 } });`
    )
  ...
  it('timeout path emits http.url, http.method, timeout_ms at error level (HTTP-03)', () => {
    const result = runLoggerScript(
      `logger.error('TestWS - HTTP Request Timeout', { workspace: { id: 'ws-1', name: 'TestWS' }, workflow: { id: 'wf-1', name: 'Flow', schema_version: '2', execution_id: 'sess-http-1', version_id: 'hist-1' }, http: { url: 'https://example.com/api', method: 'POST', timeout_ms: 10000, duration: 10001 } });`
    )
  ...
  it('success path emits at info level (HTTP-04)', () => {
    const result = runLoggerScript(
      `logger.info('TestWS - HTTP Request Executed', { workspace: { id: 'ws-1', name: 'TestWS' }, workflow: { id: 'wf-1', name: 'Flow', schema_version: '2', execution_id: 'sess-http-1', version_id: 'hist-1' }, http: { url: 'https://example.com', method: 'GET', status_code: 200, duration: 55 } });`
    )
  ...
  it('no request body, response body, or headers in http log (HTTP-05)', () => {
    const result = runLoggerScript(
      `logger.info('TestWS - HTTP Request Executed', { workspace: { id: 'ws-1', name: 'TestWS' }, workflow: { id: 'wf-1', name: 'Flow', schema_version: '2', execution_id: 'sess-http-1', version_id: 'hist-1' }, http: { url: 'https://example.com', method: 'POST', status_code: 200, duration: 50 } });`
    )
  ...
  it('failed path emits at error level with error field (HTTP-06)', () => {
    const result = runLoggerScript(
      `logger.error('TestWS - HTTP Request Failed', { workspace: { id: 'ws-1', name: 'TestWS' }, workflow: { id: 'wf-1', name: 'Flow', schema_version: '2', execution_id: 'sess-http-1', version_id: 'hist-1' }, http: { url: 'https://example.com', method: 'POST', duration: 12 }, error: 'Network error' });`
    )
    expect(result.level).toBe('error')
